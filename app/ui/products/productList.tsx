...
import { isListed, unlistProduct, relistProduct, getSellerName } from "@lib/actions";
...
export default function ProductList({sellerId = undefined, onDashboard = false}: {sellerId?: string, onDashboard?: boolean}) {
    ...
    const [listingStates, setListingStates] = useState<Record<string, boolean>>({});
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            const statuses = await Promise.all(
                fetchedProducts.map(async (product) => [product.id, await isListed(product.id)] as const)
            );
            const sellerNameEntries = await Promise.all(
                fetchedProducts.map(async (product) => [product.id, await getSellerName(product.sellerId)] as const)
            );
            setListingStates(Object.fromEntries(statuses));
            setSellerNames(Object.fromEntries(sellerNameEntries));
            setLoading(false);
        };
        fetchProducts();
    }, [page, sellerId]);

    const handleToggleListing = async (productId: string) => {
        const currentlyListed = listingStates[productId];
        setUpdatingProductId(productId);

        try {
            if (currentlyListed) {
                await unlistProduct(productId);
            } else {
                await relistProduct(productId);
            }
            setListingStates((prev) => ({ ...prev, [productId]: !currentlyListed }));
        } finally {
        ...
        <div className="space-y-4">
            <ul className="space-y-3">
                {products.map((product) => {
                    const isProductListed = listingStates[product.id];
                    const hasListingState = product.id in listingStates;
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="mt-3 flex flex-wrap items-center gap-2">
                                            {hasListingState ? (
                                                <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold ${isProductListed ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300" : "bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300"}`}>
                                                    {isProductListed ? "Publicado" : "No publicado"}
                                                </span>
                                            ) : (
                                                <span className="inline-flex items-center rounded-full bg-zinc-100 px-2.5 py-1 text-[11px] font-semibold text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400">
                                                    Cargando estado...
                                                </span>
                                            )}
                                        </div>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <button
                                            ...
                                            disabled={updatingProductId === product.id || !hasListingState}
                                            className="rounded-3xl border border-zinc-300 bg-white px-4 py-2 text-xs font-semibold text-zinc-700 shadow-sm transition hover:bg-zinc-50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:bg-zinc-800"
                                        >
                                            {updatingProductId === product.id ? "Actualizando..." : isProductListed ? "No listar" : "Listar"}
                                        </button>
                            ...
                            {onDashboard && (
                                <div className="space-y-3">
                                    ...
                                    <div className="grid gap-2 text-xs text-zinc-600 dark:text-zinc-400 bg-zinc-50 dark:bg-zinc-900 rounded-lg p-3">
                                        <div><span className="font-medium">Nombre del Vendedor:</span> {sellerNames[product.id] ?? "Cargando..."}</div>
                                        <div><span className="font-medium">Id del Vendedor:</span> {product.sellerId}</div>
                                        ...
                                        <div className="truncate"><span className="font-medium">URL Imagen:</span> <span className="break-all">{product.imageUrl}</span></div>
                                    </div>
