...
export default async function SalesReport( { sellerId=undefined }:{ sellerId?: string }) {
    ...
    const totalSalesValue = sales.map(sale => parseFloat(sale.totalPrice) * 100).reduce((sum, current) => sum + current, 0.00) / 100;
    const totalSalesValueWithoutShipping = sales.map(sale => parseFloat(sale.priceWithoutShipping) * 100).reduce((sum, current) => sum + current, 0.00) / 100;
        ...
        <div className="space-y-6">
            <div className="rounded-lg border border-zinc-100 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <div className="mt-2 space-y-1">
                    <p className="text-2xl font-semibold text-gray-900 dark:text-white">{formatCurrency(totalSalesValue)}</p>
                    <p className="text-sm text-zinc-600 dark:text-zinc-400">Sin envío: {formatCurrency(totalSalesValueWithoutShipping)}</p>
                </div>
            </div>
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="mt-3 space-y-1 text-sm text-zinc-700 dark:text-zinc-300">
                                    ...
                                    <div>
                                        ...
                                        <span className="font-medium">{formatCurrency(sale.priceWithoutShipping)}</span>
                                    </div>
                                </div>
