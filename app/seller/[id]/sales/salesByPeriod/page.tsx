...
export default async function SalesByPeriod({ params, searchParams }: { params:Promise<{ id: string }>, searchParams:SearchParams }) {
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-zinc-50 dark:bg-black px-4 py-12">
            <main className="w-full max-w-5xl rounded-2xl border border-zinc-200 bg-white p-8 shadow-lg dark:border-zinc-800 dark:bg-zinc-950">
                ...
                <section>
                    {validatedStartDate && validatedEndDate && <SalesReportByPeriod startDate={validatedStartDate} endDate={validatedEndDate} sellerId={id}/> }
                </section>
            </main>
        </div>
    );
